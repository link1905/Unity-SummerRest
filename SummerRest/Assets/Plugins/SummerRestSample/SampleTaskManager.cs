...
using DummyJsonCdn = SummerRest.Runtime.Requests.DummyJsonCdn;
namespace SummerRestSample
{
    ...
    internal class SampleTaskManager : MonoBehaviour
    {
#if SUMMER_REST_TASK
        ...
        public async UniTaskVoid GetProductData(string productId)
        {
            ...
            Debug.LogFormat("Simple response from {0}: {1}", _getProduct.AbsoluteUrl, product);

            // Detailed
            var webResponse = await _getProduct.DetailedDataRequestAsync<Product>();
            Debug.LogFormat("Detailed response from {0}: {{{1}, {2}, {3}}}", _getProduct.AbsoluteUrl,
                webResponse.StatusCode, webResponse.ContentType.FormedContentType, webResponse.Data);
        }
        ...
        public async UniTaskVoid SearchProductData(string productId)
        {
            // Set request param
            ...
            // Simple request
            var products = await _searchProduct.DataRequestAsync<Product[]>();
            var res = string.Join("; ", products.Select(e => e.ToString()).ToArray());
        ...
        public void AddProductData(Product product)
        {
            ...
            StartCoroutine(_addProduct.DataRequestCoroutine<Product>(HandleAddProductResponse));
        ...
        {
            StartCoroutine(WebRequestUtility.TextureRequestCoroutine("https://cdn.dummyjson.com/product-images/1/1.jpg",
                false, ShowImage));
        }
